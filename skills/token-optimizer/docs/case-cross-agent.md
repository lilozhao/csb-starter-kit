...
> 📄 **能力地图（一页速查）**：`docs/a2a-delegation-capability-map.md`
> （三层能力 / 三个前提 / 成本与边界 / 目标形态：模板 + UAC 授权）
...
### ✅ 已修（2026-09-12，csb-a2a-aip）

问题定位后直接改了**共享代码**（接收方 `git pull` 即可）：

| 项 | 修前 | 修后 |
|---|---|---|
| 默认注入超时 | 硬编码 90s | **5 分钟**，且可用 `A2A_BRIDGE_INJECT_TIMEOUT_MS` 覆盖 |
| 信封时限 | 不参与注入超时 | **信封 `timeoutMs` 参与计算**（取大），**封顶 15 分钟**防挂死 |
| 超时报错 | 只报“超时” | 附排查提示（调大 env / 先归档会话） |
| 测试 | 11 例 | **14 例**（新增：跟随信封 / 封顶 / 默认 5 分钟） |

> 修复思路：**超时不该是写死的常量，而应由「委托方声明的时限 + 接收方可接受上限」共同决定**——
> 但接收方始终保留封顶权（拒绝被无限拖死）。
